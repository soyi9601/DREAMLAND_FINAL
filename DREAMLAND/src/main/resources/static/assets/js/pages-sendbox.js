...
const fnGetReceiveMessage = () => {
  $.ajax({
    ...
    success: (resData) => {
      ...
      const receiveList = $('#send-list');
      const receivePage = $('#send-page');
      if (page === 1) {
        receiveList.empty();
      }
      if (resData.messageList.length === 0) {
        receiveList.append('<tr><td>보낸 쪽지가 없습니다</td></tr>');
      } else {
